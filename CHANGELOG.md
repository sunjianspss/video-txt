...
- 配音不再念出孤立的单个字母或数字。Whisper 把关门声、呼吸声写成 `x` 或 `0`，配音照着念出来比留白更
  糟。中日韩单字（`好`）、倒数（`3, 2, 1`）和 `42` 这类多字符内容照旧发声。
- `audit` 增加 `past_media_end` 告警：字幕结束点超出媒体时长 0.5 秒以上时列出具体条目。原先只检查
  反方向（字幕远早于视频结束）。这类字幕属于另一个版本的片源，配音会按视频长度截掉，播放器也永远
  显示不到。告警不阻断流程。
...
- 口语化翻译（`dub` 内部使用）不再一刀切要求缩写保持字母原样：只有目标语言日常真的念字母时才保留
  （中文的 CPU、GPS、USB），否则按目标语言的说法译成词。

### 新增：克隆参考片段改用分离出来的人声干轨

`--separate-bgm` 调用 demucs 时本来就同时产出人声和背景两条轨，之前只留背景、把人声删掉。现在人声
轨也缓存成 `你的视频.dub-cache/bgm/vocals.flac`，克隆引擎的参考片段从它上面剪，克隆到的是人声本身
而不是人声加背景音乐。

```bash
uv run --python 3.12 video-txt dub "$V" --provider lmstudio \
  --tts-engine index-tts --clone-repo ~/tools/index-tts --separate-bgm
```

- 参考片段文件名带 `-clean`（`reference/main-clean-1000.wav`），和从原始混音剪的参考分开缓存。
- 没开 `--separate-bgm`、或缓存里还没有人声轨时，照旧从原始混音剪。
- 人声轨压缩失败不影响配音，会打印一行提示后退回混音。
- 分离现在在挑音色之前跑（原先在最后混流时才跑），所以同一次运行里剪参考就能用上干轨；顺带把
  "源视频没有音轨"这类问题提前到合成语音之前报出来。

实测（20 秒人声压音乐的样片）：参考片段里只剩音乐的那 0.3 秒，从混音剪是 -38.0 dB，从干轨剪是
-85.8 dB；整段参考的语音电平基本不变（-21.6 dB / -21.8 dB）。

### 新增：只翻译变更过的字幕

`retranscribe-range` 修好几句原文后，整部影片不必重译。`--reuse` 指向修复前的原文字幕，读得一样的
行沿用旧译文，只有改过的行发给模型：

```bash
uv run --python 3.12 video-txt translate '/绝对路径/电影.repaired.srt' \
  --reuse '/绝对路径/电影.srt' --provider lmstudio
```

- 旧译文默认取 `--reuse` 旁边的 `<原文名>.<语言>.srt`，位置不同时用 `--reuse-translation` 指定。
- 匹配只看文字，忽略空白和大小写，因此重新编号和时间轴微调不影响沿用；同一句话在片中出现多次时，
  取时间上最近的那条译文。
- 沿用下来的译文同时充当新译行的上下文，语气和术语不会在修复处断开。
- 两份字幕条数对不上（不是同一次翻译的产物）时命令直接报错，不做部分猜测。
- `dub`、`run`、`mux` 的翻译阶段同样接受这两个参数。

实测（真实影片里连续 30 条字幕，本机 qwen3.5-35b-a3b）：整段重译 10.8 秒；改动其中 3 条后带
`--reuse` 重跑 3.2 秒，另外 27 条译文与首次运行逐字相同，时间轴跟着修复后的原文走。

翻译审计仍然每次审全片，这是有意的：1683 条字幕的完整审计只要 10 毫秒，增量审计省不出时间，
而全片审计还能查出修复接缝处的问题。
