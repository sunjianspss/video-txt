...
**用户只是想听、或者要放进手机**就加 `--format m4a`（体积小 4 倍，Apple Music 也认）；
默认 FLAC 是给"还要再剪进别的视频"这个用途的，无损才不会叠加两代损失。
**要拿去用在别的视频里就加 `--clean`**：分离器在对白最响的地方瑕疵最多，只有没人说话的段落
听起来才不露馅。

**慢，第一次要跑六轨分离**（几分钟）。缓存和配音共用——片子如果跑过 `--separate-bgm`，
伴奏那半是现成的。`run_in_background` 跑，别卡住会话。

**别以为"响度高就是音乐"。** 分离器把不是人声的一切都放进伴奏，而电影里大半是脚步、车流、
房间底噪。实听验证：只按响度找出的 14 段里 12 段是环境声。工具默认已经过了一道音乐性判据
（两条有音高的轨同响 ≥25% 的时间），把 14 段收敛到 2 段——**别去调低 `--min-musicality`**，
除非用户明说想要环境声。
